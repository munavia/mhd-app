...
export default function DashboardSettingsPage() {
  const t = useTranslations("Dashboard.settingsAdmin");
  ...
  const onAdd = handleSubmit(async (data) => {
    try {
      ...
      toast.error(
        e instanceof Error ? e.message : t("createError")
      );
    }
  });
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8">
      ...
      <Card className="border-border/60 shadow-sm">
        <CardHeader>
          <CardTitle className="text-lg">{t("categoriesTitle")}</CardTitle>
          <CardDescription>
            {t("categoriesDesc")}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <form onSubmit={onAdd} className="flex flex-col gap-3 sm:flex-row sm:items-end">
            <div className="min-w-0 flex-1 space-y-2">
              <Label htmlFor="category-name">{t("newCategory")}</Label>
              <Input
                id="category-name"
          ...
          <div>
            <p className="mb-3 text-sm font-medium">{t("existing")}</p>
            {loading ? (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {cat.slug} · {t("postsCount", { count: cat.postCount })}
                      </p>
                    </div>
                    <Button
                      type="button"
