  ...
  const onSubmit = async (data: ResetPasswordFormData) => {
    try {
      if (process.env.NODE_ENV === "development") {
        ...
        toast.error(json.error ?? t("resetDevError"));
        return;
      }
      ...
      <Card className="w-full max-w-md border-border/60 shadow-lg shadow-primary/5">
        <CardHeader className="space-y-1 text-center">
          {sent ? (
            ...
            <>
              <CardTitle className="font-heading text-2xl">{t("forgotTitle")}</CardTitle>
              <CardDescription>
                {t("forgotSubtitle")}
              </CardDescription>
            </>
            ...
            <>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                >
                  ...
                  {isSubmitting ? t("sendingReset") : t("sendLink")}
                </Button>
              </form>
              <Link
                href="/login"
